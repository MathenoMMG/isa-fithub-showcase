...
export function TransferStockDialog({ initialProductId, initialLoteId, customTrigger }: Props) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className={isPremium ? "max-w-2xl font-mono rounded-[4px] border-border dark:border-primary/20 bg-slate-950/95 text-slate-100" : "max-w-2xl rounded-2xl bg-white dark:bg-slate-950 border-slate-200 dark:border-slate-800 text-slate-900 dark:text-slate-100"}>
        <DialogHeader>
          <div className="flex items-center justify-between pr-6">
            ...
            <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800 p-0.5 rounded-lg text-xs">
              <Button
                ...
                size="sm"
                className={`h-7 px-3 text-xs ${activeTab === "transfer" ? "bg-emerald-600 hover:bg-emerald-700 text-white" : "text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white"}`}
              ...
              <Button
                ...
                size="sm"
                className={`h-7 px-3 text-xs ${activeTab === "history" ? "bg-emerald-600 hover:bg-emerald-700 text-white" : "text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white"}`}
        ...
        {activeTab === "transfer" ? (
          <div className="space-y-4 py-2 text-slate-900 dark:text-slate-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-11 gap-3 items-center bg-slate-50 dark:bg-slate-900 p-3.5 rounded-xl border border-slate-200 dark:border-slate-800">
              <div className="md:col-span-5 space-y-1.5">
                ...
                <Select value={String(originStoreId)} onValueChange={handleOriginStoreChange}>
                  <SelectTrigger className="h-11 bg-white dark:bg-slate-950 border-slate-300 dark:border-slate-800 text-slate-900 dark:text-slate-100 rounded-xl">
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="md:col-span-1 flex justify-center items-center py-1 md:py-0">
                <div className="h-8 w-8 rounded-full bg-emerald-100 dark:bg-emerald-950/80 text-emerald-600 dark:text-emerald-400 flex items-center justify-center">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                className="h-10 text-sm rounded-xl bg-white dark:bg-slate-950 border-slate-300 dark:border-slate-800 text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500"
              />

              <div className="max-h-44 overflow-y-auto border border-slate-200 dark:border-slate-800 rounded-xl divide-y divide-slate-100 dark:divide-slate-800 bg-white dark:bg-slate-900/60">
                    ...
                    return (
                      <div
                        ...
                        className={`flex items-center justify-between p-2.5 px-3 cursor-pointer text-xs transition-colors ${
                          isSelected
                            ? "bg-emerald-50 dark:bg-emerald-950/60 text-emerald-900 dark:text-emerald-200 font-medium"
                            : "hover:bg-slate-50 dark:hover:bg-slate-800/60 text-slate-800 dark:text-slate-200"
                        }`}
                      >
                        <div className="flex flex-col min-w-0 pr-2">
                          <span className="truncate font-semibold text-slate-900 dark:text-slate-100">{p.nombre}</span>
                          <span className="text-[10px] text-slate-500 dark:text-slate-400">SKU: {p.articulo} · {p.categoria || "Otros"}</span>
                        </div>
                        <div className="shrink-0 flex items-center gap-2">
                          <span className="px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-200 text-[11px] font-bold border border-slate-200 dark:border-slate-700">
                            Stock: {stock}
                          </span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1 animate-in fade-in duration-200">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex items-center gap-2">
                    <Button
                      ...
                      variant="outline"
                      size="icon"
                      className="h-11 w-11 rounded-xl shrink-0 border-slate-300 dark:border-slate-800 bg-white dark:bg-slate-950 text-slate-800 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800"
                    ...
                    <Input
                      ...
                      className="h-11 text-center font-bold text-base rounded-xl bg-white dark:bg-slate-950 border-slate-300 dark:border-slate-800 text-slate-900 dark:text-slate-100"
                    />
                    <Button
                      type="button"
                      variant="outline"
                  ...
                  <p className="text-[10px] text-slate-500 dark:text-slate-400 text-right">Máximo disponible: {maxAvailable} uds</p>
                </div>
                ...
                <div className="sm:col-span-2 space-y-1">
                  ...
                  <Input
                    ...
                    className="h-10 text-xs rounded-xl bg-white dark:bg-slate-950 border-slate-300 dark:border-slate-800 text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500"
                  />
                </div>
              </div>
            )}
            ...
            {selectedProduct && selectedLote && (
              <div className="p-3.5 rounded-xl bg-emerald-50/90 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800/60 text-xs space-y-1">
                <div className="flex items-center gap-1.5 font-bold text-emerald-800 dark:text-emerald-300">
                  <AlertCircle className="h-4 w-4" />
                  <span>Resumen del Traspaso:</span>
                </div>
                <p className="text-slate-800 dark:text-slate-200 leading-relaxed">
                  Se transferirán <strong className="text-emerald-700 dark:text-emerald-300 font-bold">{quantity} uds</strong> de{" "}
                  <strong className="text-slate-900 dark:text-white">"{selectedProduct.nombre}"</strong> (Vencimiento: {formatExpiryDate(selectedLote.fecha_caducidad)}) desde{" "}
                  <strong className="text-slate-900 dark:text-white">{originStoreName}</strong> hacia <strong className="text-slate-900 dark:text-white">{destStoreName}</strong>.
                </p>
              </div>
            )}
          </div>
                  ...
                  >
                    <div className="space-y-1 min-w-0 flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-emerald-100 dark:bg-emerald-950/80 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800/40">
                          {t.cantidad} uds
                        </span>
                      </div>
                      <div className="flex items-center gap-2 text-[11px] text-slate-600 dark:text-slate-400 flex-wrap">
                        <span className="font-medium text-slate-800 dark:text-slate-200">{t.origen_tienda_nombre}</span>
                        <ArrowRight className="h-3 w-3 text-slate-400" />
                        <span className="font-medium text-slate-800 dark:text-slate-200">{t.destino_tienda_nombre}</span>
                        <span>· Vence: {formatExpiryDate(t.fecha_caducidad)}</span>
                      </div>
                      {t.motivo && <p className="text-[10px] text-slate-500 dark:text-slate-400 italic">"{t.motivo}"</p>}
                    ...
                    <Button
                      ...
                      className="h-8 text-xs text-slate-500 dark:text-slate-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/30 gap-1 shrink-0"
                    >
        ...
        <DialogFooter className="gap-2 sm:gap-0 border-t border-slate-100 dark:border-slate-800/80 pt-3">
          <Button variant="ghost" onClick={() => setOpen(false)} className="rounded-xl text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800">
            Cerrar
          </Button>
          {activeTab === "transfer" && (
            <Button
              ...
              className="bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl gap-2 font-semibold shadow-sm"
            >
